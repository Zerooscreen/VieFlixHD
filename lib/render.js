...
const SITE_NAME = 'VieFlixHD';
const DEFAULT_TITLE = 'VieFlixHD · Phim & Phim Bộ Thịnh Hành: Nội Dung, Đánh Giá, Diễn Viên & Trailer Đầy Đủ';
const DEFAULT_DESC = 'VieFlixHD cung cấp nội dung phim, diễn viên, đánh giá và trailer chính thức của những phim và phim bộ phổ biến nhất hiện nay, kèm thông tin đầy đủ về mùa và tập phim.';
const DEFAULT_OG_IMAGE = 'https://placehold.co/1200x630/17171b/8d8a92?text=VieFlixHD';
...
<header>
  <div class="header-inner">
    <a class="logo" href="/movie">VieFlix<span>HD</span></a>
    <nav class="tabs">
      <a class="tab-btn ${activeTab === 'movie' ? 'active' : ''}" href="/movie">Phim</a>
      <a class="tab-btn ${activeTab === 'tv' ? 'active' : ''}" href="/tv">Phim Bộ</a>
    </nav>
    <div class="search-wrap">
      <input id="search-input" type="text" placeholder="Tìm kiếm tiêu đề..." autocomplete="off">
...
<footer>
  <p>VieFlixHD — Trang thông tin phim và phim bộ sử dụng dữ liệu công khai từ TMDB (không phải dịch vụ xem phim trực tuyến) · Powered by <a href="https://www.themoviedb.org/" target="_blank" rel="noopener">TMDB</a></p>
  ${histatsSnippet()}
</footer>
...
function posterCard(item, type) {
  ...
  return `
    <a class="poster-card" href="/${type}/${item.id}/${encodeURIComponent(slug)}?vi">
      ...
      <div class="poster-sub">${date || 'Chưa rõ năm'}</div>
    </a>
  `;
}
...
function trailerBlock(videos) {
  ...
  if (!trailer) return `<div class="no-trailer">Chưa có trailer.</div>`;
  return `
...
function castGrid(credits) {
  ...
  if (!cast.length) return `<div class="empty">Chưa có thông tin diễn viên.</div>`;
  return `<div class="cast-grid">${cast.map(c => `
    <a class="cast-card" href="/person/${c.id}/${encodeURIComponent(slugify(c.name))}?vi" style="text-decoration:none; color:inherit;">
...
function similarGrid(items, type) {
  if (!items || !items.length) return `<div class="empty">Không có nội dung tương tự.</div>`;
  const cards = items.slice(0, 6).map(item => posterCard(item, type)).join('');
  return `<div class="grid">${cards}</div>`;
}

module.exports = { head, layout, posterCard, genreRow, trailerBlock, castGrid, similarGrid, escapeHtml, movieJsonLd, tvJsonLd, sideBannerAd, nativeBannerAd, DEFAULT_TITLE, DEFAULT_DESC, SITE_NAME };
